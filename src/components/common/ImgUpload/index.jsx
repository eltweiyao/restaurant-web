  ...
  normalizeUploadValue(info) {
    ...
    fileList = fileList.map((file) => {
      ...
      if (typeof file.response === 'string') {
        newfile.response = JSON.parse(file.response);
      }
      if (file.response && file.response.success) {
        // 组件会将 file.url 作为链接进行展示,只有图片使用。
        if (file.type.indexOf('image') !== -1) {
          newfile.url = file.response.data.kbShopImgUrl;
        } else { // 上传文件时，不提供下载地址。
          newfile.tfsUrl = file.response.fileNameUrl;
        }
        newfile.id = file.response.data.image_id;
      }
      return newfile;
    });
